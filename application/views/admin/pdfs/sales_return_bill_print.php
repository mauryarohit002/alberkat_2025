...
$tbl .= <<<EOD
	<table border="1" style="text-align:center;">
		<tr>
			<td style="line-height:2;">CREDIT NOTE</td>
		</tr>
	</table>
EOD;

$inv_no = $sale_return_data[0]['srm_inv_no'];
$inv_date = $sale_return_data[0]['srm_inv_date'];
$client_name = $sale_return_data[0]['customer'];
$client_address = $sale_return_data[0]['cust_address'];
$client_mob = $sale_return_data[0]['tel_1'];
$client_gst_no = $sale_return_data[0]['gst_no'];
$client_pan_no = $sale_return_data[0]['pan'];
$tbl .= <<<EOD
...
EOD;
			...
			foreach($sale_return_trans as $key => $value) 
			{
				...
				$qty = $value['srt_qty'];
				$rate = $value['srt_rate'];
				$amt = $value['srt_sub_amt'];
				$disc = $value['srt_disc_amt'];
				$t_value = $value['srt_taxable_amt'];
				$sgst = $value['srt_sgst_per'];
				$sgst_amt = $value['srt_sgst_per_amt'];
				$cgst = $value['srt_cgst_per'];
				$cgst_amt = $value['srt_cgst_per_amt'];
				$igst = $value['srt_igst_per'];
				$igst_amt = $value['srt_igst_per_amt'];
				$final_amt = $value['srt_final_total'];				
...
EOD;
	...
	$net_amt = $sale_return_data[0]['srm_gross_amt'];
	$disc_amt = $sale_return_data[0]['srm_disc'];
	$b_sgst_amt = $sale_return_data[0]['srm_sgst_amt'];
	$b_cgst_amt = $sale_return_data[0]['srm_cgst_amt'];
	$b_igst_amt = $sale_return_data[0]['srm_igst_amt'];
	// $b_sgst_amt = $sale_return_data[0]['sm_igst_amt'];
	$b_final_total = $sale_return_data[0]['srm_net_amt'];
